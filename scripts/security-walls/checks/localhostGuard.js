...
function referencesGuard(configSource) {
  const match = /\bglobalSetup\s*:\s*(\[[^\]]*\]|(['"`])[^'"`]*\2)/.exec(stripComments(configSource))
...
export const localhostGuard = {
  ...
  run(ctx) {
    ...
    if (!existsSync(guardPath)) {
      results.push({ status: 'fail', message: 'guard file is missing', file: GUARD_FILE })
    } else {
      const remote = runGuard(guardPath, REMOTE_URL)
      results.push(
        remote.code === 1
          ? { status: 'pass', message: `stops with exit 1 for ${REMOTE_URL}`, file: GUARD_FILE }
          : {
              status: 'fail',
      ...
      results.push(
        local.code === 0
          ? { status: 'pass', message: `lets ${LOCAL_URL} through`, file: GUARD_FILE }
          : {
              status: 'fail',
    ...
    if (!existsSync(join(ctx.root, CONFIG_FILE))) {
      results.push({ status: 'fail', message: 'integration Vitest config is missing', file: CONFIG_FILE })
    } else if (referencesGuard(ctx.readFile(CONFIG_FILE))) {
      results.push({ status: 'pass', message: `uses ${GUARD_FILE} as globalSetup`, file: CONFIG_FILE })
    } else {
      results.push({ status: 'fail', message: `does not reference ${GUARD_FILE} as globalSetup`, file: CONFIG_FILE })
    }
    return results
  },
}
