...
- [ ] `--linked` verilmediğinde tek bir SKIP sonucu ("production kontrolü için --linked kullan") dönüyor
- [ ] CLI login/link yoksa ya da komut başarısız olursa SKIP ve nasıl bağlanılacağına dair kısa mesaj; FAIL değil
- [ ] Local'de olup production `schema_migrations`'ta olmayan versiyon FAIL; production'da olup local'de olmayan versiyon FAIL
- [ ] Production `pg_policies`'te Duvar 1'in beklediği bir policy eksikse ya da ifadesinde `auth.uid()` / beklenen üst tablo yoksa FAIL
- [ ] Çalıştırılan her Supabase CLI komutu izinli salt-okunur listeden geçiyor; listede olmayan bir komut denemesi exception'la reddediliyor
- [ ] Sabit örnek CLI çıktılarıyla (eşleşen, eksik versiyon, eksik policy) testler var
- [ ] `--linked` olmadan gerçek repoda bu duvar SKIP veriyor
