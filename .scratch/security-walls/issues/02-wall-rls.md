...
- [ ] Migration'larda oluşturulan her tabloda RLS açık değilse FAIL (tablo adı + ilgili migration dosyası)
- [ ] İstisna listesinde olmayan, RLS'i açık ama hiç policy'si olmayan tablo FAIL
- [ ] Policy'lerinin hiçbirinin `using`/`with check` ifadesinde `auth.uid()` geçmeyen tablo FAIL
- [ ] Config'teki sahiplik beklentisine göre policy ifadesinde beklenen üst tablo(lar) referans verilmiyorsa FAIL; `book_libraries` için `libraries` referansının eksik olduğu durum (migration 011 regresyonu) ayrıca test edildi
- [ ] `set search_path` içermeyen ya da istisna listesinde olmayan `security definer` fonksiyon FAIL
- [ ] Her kural için küçük bir kötü örnek fixture'la FAIL veren test var
